  ...
  desUpload = function(ele){
    ...
    $ele.each(function(index){
      ...
      var uploader = new plupload.Uploader({
        ...
        url : baseurl.root+'app/file/uploadauto',//服务器端的上传页面地址
        resize: {
      	  quality: 90,
      ...
      uploader.bind('FileUploaded',function(uploader,file,responseObject){
        ...
        console.log(baseurl.root+data.cutfilename);
          $( '#'+file.id ).find('img').attr('src',baseurl.root+data.cutfilename).attr({'data-compressfilename':data.compressfilename,'data-filename':data.filename});
      });
